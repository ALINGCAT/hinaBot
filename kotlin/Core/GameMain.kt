...
class GameMain {
    ...
    fun isFirstSignIn(qq:Long):Boolean {
        return if (Player.instances[qq] == null) {
            EssDao.addPlayer(qq)
            Player.instances.put(qq, Player(qq, 50, LocalDateTime.now(), 1))
            true
        }
        else false
    }
    ...
    fun getLot(qq:Long): HashMap<String, String> {
        ...
        result.put("total", Player.instances[qq]!!.coins.toString())
        result.put("accumulation", Player.instances[qq]!!.accumulation.toString())
        return result
    }
    fun draw(qq: Long): Item? {
        ...
        EssDao.setCoins(qq, Player.instances[qq]!!.coins - 50)
    ...
    fun drawTenTimes(qq: Long): HashMap<Item, Int>? {
        ...
        EssDao.setCoins(qq, Player.instances[qq]!!.coins - 500)
